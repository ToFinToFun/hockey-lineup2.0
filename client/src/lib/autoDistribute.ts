// Auto-fördela v2: Optimal speltidsbalans via calculateDistributions
//
// Prioriteringsordning:
// 1. Favoritposition – MV→MV, B→B, C→C, F→F, IB→resten
// 2. Lagfärg – white→team-a, green→team-b (MV låses till sin lagfärg)
// 3. Jämna lag – lika många spelare per lag (±1)
// 4. Speltidsbalans – minimera skillnad i istid mellan positioner
//
// Regler:
...
// - Shuffle-läge: omfördelar neutrala spelare men behåller lagfärg-låsta
...
export function autoDistribute(
  ...
  const gkNeutral = options?.shuffle ? shuffleArray(goalkeepers.filter(t => !t.player.teamColor)) : goalkeepers.filter(t => !t.player.teamColor);
  ...
  // (only move players WITHOUT teamColor)
  const rebalance = (bigger: TaggedPlayer[], smaller: TaggedPlayer[]) => {
    while (bigger.length - smaller.length > 1) {
      // Find a neutral player in the bigger team to move
      const idx = bigger.findIndex(t => !t.player.teamColor);
      if (idx === -1) break; // Can't move anyone with teamColor
  ...
  function fillTeam(slots: Slot[], goalkeepers: TaggedPlayer[], outfield: TaggedPlayer[]) {
    ...
    for (let i = 0; i < gkSlots.length && i < goalkeepers.length; i++) {
      lineup[gkSlots[i].id] = goalkeepers[i].player;
      placed.add(goalkeepers[i].player.id);
    }

    // 5b: Separate outfield by preferred position
    const defenders = outfield.filter(t => t.posType === "defense");
    const centers = outfield.filter(t => t.posType === "center");
    const forwards = outfield.filter(t => t.posType === "forward");
    const flexPlayers = outfield.filter(t => t.posType === "flex");
    ...
    // Priority: flex first, then overflow from other positions
    ...
    // Collect all unplaced in priority order
